...
## Mark specific cells in an astra_table as containing pre-rendered markup
## (e.g. literal LaTeX) that any print backend must pass through as-is
## rather than escape as free text.
.astra_mark_raw_markup = function(df1, col, rows) {
  marks = attr(df1, "raw_markup")
  if (is.null(marks)) marks = list()
  marks[[col]] = union(marks[[col]], rows)
  attr(df1, "raw_markup") = marks
  df1
}

.astra_raw_markup_rows = function(df1, col) {
  attr(df1, "raw_markup")[[col]]
}
...
.astra_producer = function() {
  ...
  list(
    package = package
    , keywords = "effect size, R, {package}" %>% glue()
    ...
    ## !!!
    , citation_author = "[REDACTED]. "
    , citation_title = "es4all: Effect Sizes for All Data Types with CI's and P-Values"
    , citation_suffix = ". [REDACTED], R package version {version}, <{url}>."
    , url = "[REDACTED]"
  )
}
